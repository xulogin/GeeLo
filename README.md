...
:: 1) 双击 测试台\安装.bat            查 Node → 装依赖 → 跑自检
:: 2) 拿一次 GEE 凭据（只需一次）
...
然后**在 GeeLo 目录里开你的 AI 助手，直接提需求**——
`AGENTS.md` 会被自动读取，不用交代背景。

装不上？把 `环境自检.bat` 的完整输出丢给你的 AI 助手，
它有足够信息判断（这也是那些文档写这么细的原因）。

详细步骤见第五节，日常用法见第六节。

| 我想…… | 看这里 |
|---|---|
| 知道它为什么值得装 | [一](#一不是-ai-笨是它写完没人批改)、[三](#三还有大招跑通--对) |
| 装起来 | [五](#五装起来) |
| 日常怎么用 | [六](#六日常怎么用) |
| 知道它凭什么能在本机跑 GEE | [七](#七它到底怎么做到的一句话原理)、`测试台\原理.txt` |
| 确认它不会乱动我的账号和数据 | [八、安全边界](#八安全边界) |
...
**Haifeng Xu**　<xuhf@swfu.edu.cn>

许可：**MIT**（见 `LICENSE`）。可以自由使用、修改、分发，包括商用，
只要保留版权声明和许可声明即可。
